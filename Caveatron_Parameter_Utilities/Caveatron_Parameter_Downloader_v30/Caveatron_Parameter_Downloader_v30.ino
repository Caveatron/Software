...
void DownloadBoxDimensions() {
  int8_t sByte;
  Serial.println("Box Dimensions");
  for (int i=0; i<7; i++) {
...
void DownloadPreferences() {
  ...
  Serial.print(sByte); Serial.print(",");
  sByte = EEPROM.read(ADDR_LIDARWIN_PREF);
  Serial.print(sByte); Serial.print(",");
  ...
  Serial.print(sByte); Serial.print("-");
  sByte = EEPROM.read(ADDR_MANUAL_MODE_SET);
  Serial.println();
  Serial.println();
}
