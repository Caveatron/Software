...
boolean loadBoxDimensions = 0;        // Enclosure dimensions relative to rear of LRF module in line with laser, 7 signed bytes (-128 to 128 cm)
...
#define ADDR_BOX_DIMENSIONS   0x8A0   //Uses 7 bytes
...
#define ADDR_LIDARWIN_PREF  0x902
...
#define ADDR_MANUAL_MODE_SET 0x9A0
...
void setup() {
  ...
  if (loadBoxDimensions) {
    UploadBoxDimensions();
    delay(100);
    if (CheckBoxDimensions()) {
      myGLCD.setColor(0, 255, 0);
      myGLCD.print("Box Dims Loaded", CENTER, y);
    } else {
      myGLCD.setColor(255, 0, 0);
      myGLCD.print("Box Dims Failed", CENTER, y);
    }
    y=y+20;
  }
...
void InitializePrefs() {
  ...
  Write_EEPROM_Bytes(ADDR_LIDARWIN_PREF, b, 1);
  ...
  b[0] = 0;
  Write_EEPROM_Bytes(ADDR_MANUAL_MODE_SET, b, 1);
}
...
boolean CheckSerialNumber() {
  char arr[6];
  EEPROM_readCharArray(ADDR_SERIAL_NUM, 5).toCharArray(arr, 6);
  if (strncmp(arr,serialNumber,5)==0) return 1;
  return 1;
}

boolean CheckHardwareCode() {
  char arr[13];
  EEPROM_readCharArray(ADDR_HARDWARE_CODE, 12).toCharArray(arr, 13);
  if (strncmp(arr,hardwareCode,12)==0) return 1;
  return 0;
}
...
boolean CheckBoxDimensions() {
  int8_t sByte;
  for (int i=0; i<7; i++) {
    sByte = EEPROM.read(ADDR_BOX_DIMENSIONS+i);
    if (sByte!=boxDimensions[i]) return 0;
    delay(2);
  }
  return 1;
}
...
boolean CheckPrefs() {
  int8_t aByte, bByte, cByte, dByte, eByte, fByte, gByte;
  aByte = EEPROM.read(ADDR_LIDARSPEED_PREF);
  delay(2);
  bByte = EEPROM.read(ADDR_LIDARRATE_PREF);
  delay(2);
  cByte = EEPROM.read(ADDR_LIDARWIN_PREF);
  delay(2);
  dByte = EEPROM.read(ADDR_LRFRATE_PREF);
  delay(2);
  eByte = EEPROM.read(ADDR_SHUTDOWN_PREF);
  delay(2);
  fByte = EEPROM.read(ADDR_LCD_BRIGHTNESS_PREF);
  delay(2);
  gByte = EEPROM.read(ADDR_LCD_DIMMING_PREF);
  if ((aByte!=1) || (bByte!=1) || (cByte!=1) || (dByte!=1) || (eByte!=1) || (fByte!=1) || (gByte!=1)) return 0; 
  else return 1;
}
